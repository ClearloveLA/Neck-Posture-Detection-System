...
from modules.get_parameters import get_parameters_conv, get_parameters_bn, get_parameters_conv_depthwise, get_parameters_effi
...
#           num_workers, checkpoint_path, weights_only, from_parc, checkpoints_folder, log_after,
#           val_labels, val_images_folder, val_output_name, checkpoint_after, val_after, opts):
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
          num_workers, checkpoints_folder, log_after,
          val_labels, val_images_folder, val_output_name, checkpoint_after, val_after, opts):
    net = PoseEstimationWithEffi(num_refinement_stages)
    ...
    optimizer = optim.Adam([
        ...
        {'params': get_parameters_effi(net.model), 'lr': base_lr},
        # {'params': get_parameters_effi(net.conv1x1), 'lr': base_lr},
    ...
    checkpoint = torch.load('models/adv-efficientnet-b0.pth', map_location='cpu')
    # 删除全连接部分的weight和bias
    # print(checkpoint)
    del checkpoint['_fc.weight']
    del checkpoint['_fc.bias']
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--from-parc', action='store_true',
    #                     help='load weights from ParcNet feature extractor')
    ...
    #       args.batches_per_iter, args.num_workers, args.checkpoint_path, args.weights_only, args.from_parc,
    #       checkpoints_folder, args.log_after, args.val_labels, args.val_images_folder, args.val_output_name,
    #       args.checkpoint_after, args.val_after, args)
    train(args.prepared_train_labels, args.train_images_folder, args.num_refinement_stages, args.base_lr, args.batch_size,
          args.batches_per_iter, args.num_workers, 
          checkpoints_folder, args.log_after, args.val_labels, args.val_images_folder, args.val_output_name,
          args.checkpoint_after, args.val_after, args)
